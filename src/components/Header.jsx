  ...
  return (
    <>
      ...
      >
        ...
        {/* メニュー本体 */}
        <div className="absolute top-0 right-0 w-[75%] h-full bg-[#f5f3ef] px-8 py-24 flex flex-col">

          {/* CLOSE */}
          <button
            onClick={() => setIsOpen(false)}
            className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center z-[200]
              rounded-full bg-white/30 backdrop-blur-md border border-white/40
              opacity-80 hover:opacity-100 transition-all duration-500 ease-out
          ...
          <nav className="flex flex-col space-y-4 text-[#5a6b74]">
            ...
            <NavLink to="/courses" className={navItemClass} onClick={() => setIsOpen(false)}>
              <span translate="no">Courses</span>
            </NavLink>

            <NavLink to="/price" className={navItemClass} onClick={() => setIsOpen(false)}>
              <span translate="no">Price</span>
            </NavLink>
          ...
          <div className="mt-8">
            ...
            >
              {/* 翻訳防止：屋号 */}
              <span translate="no" className="text-[13px] tracking-[0.15em]">
                hico
              </span>
